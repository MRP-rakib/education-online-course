...
function ShowAllCourse() {
  ...
  const showCount = indexOfLastItem>courses.length?courses.length:indexOfLastItem
  return (
    <div className="mt-8 md:mt-12 lg:mt-16">
      <Container>
        <p>Show {showCount} <span className="text-blue-600">Course</span> of {courses.length} result</p>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7.5 my-10">
          {currentCourses.map(course => (
            <CoursesCard key={course.id} image={course.image} title={course.title} totalHours={course.totalHours}
              totalLessons={course.totalLessons} totalStudents={course.totalStudents}
              category={course.title} price={course.price} instructor={course.instructor}
              avater={course.avater} rating={course.rating} />
          ))}
        </div>
