...
const InstructorsCard = ({
  ...
  return (
    <div key={id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden group">
      <div className="p-8 flex flex-col items-center text-center">
          ...
          <div className="absolute -right-2 -bottom-2 flex items-center">
            <div className="relative overflow-hidden rounded-full transition-all duration-300 ease-linear w-8 h-8 group-hover:w-28">
              ...
              <div className="absolute right-0 top-0 h-8 pr-1 flex items-center gap-2 transform translate-x-6 opacity-0 group-hover:translate-x-0 group-hover:opacity-100 transition-all duration-300">
                <Link href={facebook||'#'} target='_blank' aria-label="Facebook" className="w-7 h-7 rounded-full bg-white flex items-center justify-center shadow-sm">
                  <Facebook className="text-blue-600" size={14} />
                </Link>
                <Link href={linkedin||'#'} target='_blank' aria-label="LinkedIn" className="w-7 h-7 rounded-full bg-white flex items-center justify-center shadow-sm">
                  <Linkedin className="text-blue-600" size={14} />
                </Link>
                <Link href={twitter||'#'} target='_blank' aria-label="Twitter" className="w-7 h-7 rounded-full bg-white flex items-center justify-center shadow-sm">
        ...
        <p className="mt-1 text-sm text-blue-600 capitalize ">{department}</p>
        {/* <p className="mt-1 text-sm text-start">{description}</p> */}
      </div>
